...
static uint32_t cycles_to_ms_x10(uint32_t cycles) {
  uint64_t scaled = ((uint64_t)cycles * 10000ULL) + ((uint64_t)SYSTEM_SYSCLK_HZ / 2ULL);
  return (uint32_t)(scaled / (uint64_t)SYSTEM_SYSCLK_HZ);
}
...
int main(void) {
  ...
  printf("\n=== MAX6675 + microSD raw ring logger (O1 boot) ===\n");
  ...
  uint32_t init_cycles = DWT->CYCCNT - t0_cycles;
  if (!dwt_was_enabled) {
    DWT->CTRL &= ~DWT_CTRL_CYCCNTENA_Msk;
  }
  uint32_t init_ms_x10 = cycles_to_ms_x10(init_cycles);
  printf("microSD init time: %" PRIu32 ".%" PRIu32 " ms (%" PRIu32 " cycles)\n",
         init_ms_x10 / 10U,
         init_ms_x10 % 10U,
         init_cycles);
