...
// Run a chip-select scoped receive operation.
static inline bool spi_cs_receive(spi_xfer_cs_fn_t cs_enable_fn,
                                  spi_xfer_cs_fn_t cs_disable_fn,
                                  spi_xfer_rx_fn_t rx_fn,
                                  uint8_t *rx_data,
                                  uint32_t rx_size) {
  if ((cs_enable_fn == 0U) || (cs_disable_fn == 0U) || (rx_fn == 0U)) {
    return false;
  }

  cs_enable_fn();
  bool ok = rx_fn(rx_data, rx_size);
  cs_disable_fn();
  return ok;
}
