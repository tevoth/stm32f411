...
centralized in \texttt{fatfs\_fopen()} and \texttt{fatfs\_fprintf()}.
...
while (1) {
  ...
  if (n > 0) {
    printf("t=%" PRIu32 "ms ...\n", ...); // UART remains active
    if (sd_logging_enabled && !fatfs_fprintf(&log_file, line)) {
...
\item header: \texttt{time\_ms,ax\_mg,ay\_mg,az\_mg}
\item rows: one timestamped sample per line with CRLF endings
\end{itemize}
